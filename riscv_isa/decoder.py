...
val_inst = 1           ** is this instruction valid? always set to 1 **
br_type = 0
op1_sel = 0
op2_sel = 0
ALU_fun = 5
wb_sel = 1
rf_wen = 1
mem_em = 0              ** not on the diagram, set to 1 for memory ops (lw / sw) **
mem_wr = 0              ** on the diagram this is mem_rw read=0 write=1 **
mask_type = 0           ** on the diagram this is mem_val **
csr_cmd = 3             
...
** there are more exmaples at the bottom of this file, and you can run it for a dmeo***

"""

"""
These enums map each control signal name to it's numeric value. 
"""
...
# this is the raw compressed control table using the edited enums, this is generated by *construct()*, do not edit!
...
class IControl(dict):
    ...
    def __str__(self, extfmt= False): 
        # extfmt is the "old" format, saved for compatibility
        if extfmt:
            return " ".join(["{}:{:8}".format(i.rjust(8), self.renums[i][getattr(self,i)]) for i in self.fields])
        else:
            return ", ".join([f"{self.renums[i][getattr(self,i)].rjust(len(i))}" for i in self.fields])
...
def get_control():
    ...
    return control, renum

def print_control(control = None):
    if control == None:
        control, _rev = get_control()

    # header
    print(f"{'instr'.rjust(8)}, ", end="")
    print(", ".join(_rev.keys()))
    print("-"*106)
    # print the table
    for instr, ctrl in control.items():
        print(f"{instr.rjust(8)}: {ctrl}")


if __name__ == "__main__":
    # get the control table, you are now in control!
    # the first return is the actual control table
    # the second is a reverse lookup of control value to string name, if you want that (not necessary)
    c, rev = get_control()
    ...
    # example to show some control signals.
    print ("Control DEMOS:")
    
    # let's print the supported instructions
    print("Control supports: ", ", ".join(c.keys()))
    
    # the fisrt level is the instruction name (pseudo ops must be mapped to the real instruction)
    print(f"add's op1_sel value is: {c['add']['op1_sel']}")
    # you can also access control signals as a attribute
    print(f"You can also access the same value this way: {c['add'].op1_sel}")

    op1_sel_val = c['add'].op1_sel
    print(f"But what does 0 *mean*? It means: " + rev['op1_sel'][op1_sel_val])




    print("The possible values for op1_sel are: ", ",".join(map(str, rev['op1_sel'].keys())))

    print("The possible values for op1_sel are: ", ",".join(rev['op1_sel'].values()))

    print("Or you can just inspect the dictionary", str(rev['op1_sel']))
