...
if len(sys.argv) >= 2: mem_address = str(sys.argv)[1]
...
for t in itertools.count():
   ...
   print(ALU.alu(op1, op2, ControlSignals.get_alufun()))
   ...
   rd_val = RegFile.reg_vals[rd_index]
   
   print(f"rd: {rd_val} [x{rd_index}]   " + \
         f"rs1: {rs1_val} [x{rs1_index}]   " + \
         f"rs2: {rs2_val} [x{rs2_index}]   "
   )
